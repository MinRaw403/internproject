        ...
        axios.get('http://localhost:5000/api/items')
            .then(response => {
                if (response.data.success) {
                    ...
                    if (deletedItemCode) {
                        fetchedItems = fetchedItems.filter(item => item.itemCode !== deletedItemCode);
                    }
            ...
            .then(response => {
                if (response.data.success) {
                    ...
                    if (!formData.category && response.data.categories.length > 0) {
                        setFormData(prev => ({ ...prev, category: response.data.categories[0].code }));
                    }
                }
            })
            .catch(error => {
            ...
            .then(response => {
                if (response.data.success) {
                    ...
                    if (!formData.supplier && response.data.suppliers.length > 0) {
                        setFormData(prev => ({ ...prev, supplier: response.data.suppliers[0].code }));
                    }
                }
            })
            .catch(error => {
                        ...
                        <div className="form-row">
                            <div className="form-group">
                                ...
                                <div className="file-upload-group">
                                    <input type="text" id="imagePath" name="imagePath" value={formData.imageFile ? formData.imageFile.name : ''} readOnly placeholder="Image path" />
                ...
                <section className="detail-view-section">
                    ...
                    <div className="table-container">
                        <table>
                            ...
                            <tbody>
                            {items.map((item) => (
                                <tr key={item.itemCode} className="clickable-row" onClick={() => handleRowClick(item)} style={{ cursor: 'pointer' }}>
                                    <td>{item.category}</td>
                                    <td>{item.itemCode}</td>
            ...
            <footer className="footer">
                <div className="footer-content">
                    <div className="footer-left">
                        ...
                        <p>Built with precision, powered by innovation. Delivering cutting-edge IT solutions and business solutions for the future of business.</p>
                    </div>
                ...
                <div className="copyright">
                    <p>Copyright 2023 © Proxima Technologies Pvt Ltd. All Right Reserved.</p>
                </div>
            </footer>
        </div>
        </div>
