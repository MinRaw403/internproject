...
app.get("/api/items/search", async (req, res) => {
  try {
    ...
    const items = await Item.find({
      $or: [{ itemCode: regex }, { description: regex }, { rackNumber: regex }],
...
app.put("/api/items/:itemCode", upload.single("image"), async (req, res) => {
  const { itemCode } = req.params
  const { category, unitPrice, unit, rackNumber, supplier, reOrder, description } = req.body
...
app.put("/api/categories/:code", async (req, res) => {
  ...
  console.log("📝 Updating category:", code, "with description:", description)
  ...
  try {
    ...
    if (!updatedCategory) {
      console.log("❌ Category not found with code:", code)
...
const generateGrnNo = async () => {
  ...
  const lastNo = Number.parseInt(lastGrn.grnNo.replace("GRN-", ""), 10)
...
app.get("/api/dashboard", async (req, res) => {
  try {
    ...
    const items = await Item.find()
    const totalStockValue = items.reduce((sum, i) => sum + (Number.parseFloat(i.unitPrice) || 0), 0)
...
app.get("/api/report-suppliers", async (req, res) => {
  try {
    ...
    const summary = suppliers.map((s) => {
      const supplierOrders = purchaseOrders.filter((po) => po.supplier?.name === s.name)
